...
void handleBluetooth() {
  if (btSerial.available()) {
    ...
    // 공백으로 구분된 6개의 값인지 확인
    if (cmd.indexOf(' ') != -1) {  // 공백이 있으면 다중 값
      int values[6] = {0};  // 6개의 값을 저장할 배열
      int index = 0;
      int start = 0;
      ...
      for (int i = 0; i < cmd.length(); i++) {
        if (cmd[i] == ' ' || i == cmd.length() - 1) {
          String valueStr;
          if (i == cmd.length() - 1) {  // 마지막 값 처리
            valueStr = cmd.substring(start);
          } else {
            valueStr = cmd.substring(start, i);
          }
          values[index++] = valueStr.toInt();  // 값을 정수로 변환
          start = i + 1;  // 다음 값 시작 위치
          if (index >= 6) break;  // 6개 초과하지 않도록
        }
      }
      ...
      Serial.println("Schedule data sent via I2C");
    } else {  // 단일 값 처리
      byte singleCmd = cmd.toInt();
      switch (singleCmd) {
